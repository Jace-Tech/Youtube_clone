import { Box, Flex, LinkBox, Circle, HStack, IconButton, Avatar, Badge } from '@chakra-ui/react'
...
import avaterIcon from "../assets/img/jace.png"
        ...
        <Box w={"100%"}>
            ...
            <Flex alignItems={"center"} py={2} pr={10} justifyContent={"space-between"} w={"100%"}>
                ...
                <Flex maxW={600} flex={1} alignItems={"center"}>
                    ...
                    <Circle ml={3} w={45} h={45} bgColor={"gray.800"} cursor={"pointer"} rounded={"full"}>
            ...
            {/* Category / Tag Nav */}
            {
                pathname === "/" && (
                    <Box overflow={"hidden"}>
                        <HStack className={"scrollbar"} overflowX={"scroll"} py={2} px={5} borderBottom={1} borderTop={1} borderStyle={"solid"} borderColor={"gray.600"}>
                            {
                                tags.map((item, index) => (
                                    <LinkBox key={index} onClick={() => changeCurrentTag(item)}>
                                        <Link to={item.link}>
                                            <Badge border={1} fontSize={".7rem"} bgColor={item.isActive ? "white" : "gray.600"} color={item.isActive ? "black" : "white"} borderStyle={"solid"} borderColor={"gray.500"} rounded={"full"} py={2} px={4}>
                                                {item.name}
                                            </Badge>
                                        </Link>
                                    </LinkBox>
